...
Both `action` and `resource` use dewcode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.dewcode` files is intentionally deferred until `.dewcode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether dewcode permits an operation through its managed execution paths.
