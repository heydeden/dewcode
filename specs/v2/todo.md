...
The dewcode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@dewcode-ai/llm` tool loop after replacing its
...
- plugins get global "dewcode" instance like in that post i showed
- dewcode instance has stuff like `dewcode.session.prompt()` or
  `dewcode.tool.register({...})`
...
- decide whether to preserve deprecated `@dewcode-ai/llm` orchestration exports
