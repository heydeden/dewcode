...
Move the canonical DEWCode terminal application from
`packages/dewcode/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/dewcode`,
`packages/cli`, or `@dewcode-ai/core`.

The SDK is the TUI's DEWCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/dewcode`.
- Do not replace private `packages/dewcode` imports with `@dewcode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@dewcode-ai/tui`.
...
- DEWCode domain data displayed by the TUI
...
```text
packages/dewcode/src/cli/cmd/tui
```

Its private dependency on `packages/dewcode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/dewcode/src/*`.
...
Status: Completed. The private `@dewcode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@dewcode-ai/tui`.
...
- The package has no dependency on `dewcode`, `@dewcode-ai/cli`, or
  `@dewcode-ai/core`.
...
primitives now live in `@dewcode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@dewcode-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads DEWCode globals or process state;
...
Replace process-global DEWCode state with resolved TUI inputs.
...
`@dewcode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/dewcode` initially.
...
canonical in `@dewcode-ai/tui`. Configured references resolve through the new
...
adapters rather than DEWCode domain implementations.

Make the SDK the only DEWCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports DEWCode data and operations only from
  `@dewcode-ai/sdk`.
...
`@dewcode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/dewcode`.
...
- The package has no imports from `packages/dewcode`, `packages/cli`, or
  `@dewcode-ai/core`.
...
public `@dewcode-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/dewcode`.
- Keep the legacy embedded worker and server startup in `packages/dewcode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@dewcode-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/dewcode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@dewcode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@dewcode-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for DEWCode domain state.
...
cd packages/dewcode && bun typecheck
...
rg '@dewcode-ai/core|packages/dewcode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/dewcode/src packages/dewcode/test
```
...
- Build the current-platform `packages/dewcode` binary.
