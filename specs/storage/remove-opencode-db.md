# Remove `packages/dewcode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/dewcode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the dewcode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/dewcode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/dewcode/src/account/repo.ts`
- `packages/dewcode/src/cli/cmd/db.ts`
- `packages/dewcode/src/cli/cmd/import.ts`
- `packages/dewcode/src/cli/cmd/stats.ts`
- `packages/dewcode/src/control-plane/workspace.ts`
- `packages/dewcode/src/index.ts`
- `packages/dewcode/src/node.ts`
- `packages/dewcode/src/permission/index.ts`
- `packages/dewcode/src/project/project.ts`
- `packages/dewcode/src/server/projectors.ts`
- `packages/dewcode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/dewcode/src/server/shared/fence.ts`
- `packages/dewcode/src/session/message-v2.ts`
- `packages/dewcode/src/session/projectors.ts`
- `packages/dewcode/src/session/prompt.ts`
- `packages/dewcode/src/session/session.ts`
- `packages/dewcode/src/session/todo.ts`
- `packages/dewcode/src/share/share-next.ts`
- `packages/dewcode/src/storage/db.ts`
- `packages/dewcode/src/sync/index.ts`
- `packages/dewcode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy dewcode database wrapper; `packages/dewcode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the dewcode wrapper.
...
Status: Completed. `SyncEvent` and the dewcode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy dewcode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy dewcode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy dewcode database wrapper.
...
All migration groups are complete or superseded. `packages/dewcode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No dewcode data-migration group remains.

The previous dewcode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate dewcode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/dewcode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/dewcode/src`
- `bun typecheck` from `packages/dewcode`
- Relevant package tests from `packages/dewcode`, not the repo root
