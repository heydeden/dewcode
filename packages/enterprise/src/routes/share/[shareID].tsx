import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@dewcode-ai/sdk/v2"
import { SessionTurn } from "@dewcode-ai/session-ui/session-turn"
import { SessionReview } from "@dewcode-ai/session-ui/session-review"
import { DataProvider } from "@dewcode-ai/session-ui/context"
import { FileComponentProvider } from "@dewcode-ai/ui/context/file"
import { WorkerPoolProvider } from "@dewcode-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@dewcode-ai/ui/logo"
import { IconButton } from "@dewcode-ai/ui/icon-button"
import { ProviderIcon } from "@dewcode-ai/ui/provider-icon"
import { iife } from "@dewcode-ai/core/util/iife"
import { Binary } from "@dewcode-ai/core/util/binary"
import { NamedError } from "@dewcode-ai/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@dewcode-ai/ui/tabs"
import { MessageNav } from "@dewcode-ai/session-ui/message-nav"
import { FileSSR } from "@dewcode-ai/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@dewcode-ai/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/dewcode-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | DEWCode</Title>
              </Show>
              <Meta name="description" content="dewcode - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/dedenwirjadinata/dewcode"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://dewcode.dev/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
