...
export const DEWCODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const DEWCODE_MODE_KEY = "dewcode.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(DEWCODE_MODE_KEY, DEWCODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(DEWCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(DEWCODE_MODE_KEY, stack.at(-1)?.mode ?? DEWCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? DEWCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(DEWCODE_MODE_KEY, undefined)
