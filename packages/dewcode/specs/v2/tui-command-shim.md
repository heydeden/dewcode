...
Delete `packages/dewcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/dewcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/dewcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/dewcode`
- TUI plugin loader tests in `packages/dewcode` if runtime plugin API wiring changed
