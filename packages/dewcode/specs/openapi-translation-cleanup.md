...
Trim `packages/dewcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/dewcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/dewcode`.
- `[x]` `bun typecheck` from `packages/dewcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/dewcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/dewcode`.
- `[x]` `bun dev generate > /tmp/dewcode-openapi.json` from `packages/dewcode`.
...
- Edit `packages/dewcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/dewcode-openapi.json` from `packages/dewcode`.
...
- Check source schemas in files like `packages/dewcode/src/session/schema.ts`, `packages/dewcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/dewcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/dewcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
