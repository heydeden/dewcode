...
Practical reference for a future `packages/server` split after the dewcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/dewcode`.
...
Until enough shared service code lives outside `packages/dewcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/dewcode`

It should not import `packages/dewcode` services while `packages/dewcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on dewcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/dewcode` implementation details.
