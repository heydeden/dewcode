...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-dewcode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-dewcode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://dewcode.dev/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-dewcode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-dewcode-workspace")
