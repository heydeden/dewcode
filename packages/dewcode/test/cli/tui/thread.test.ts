...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ dewcode }) =>
    Effect.gen(function* () {
      const result = yield* dewcode.spawn(["--replay-limit", "10"])

      dewcode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ dewcode }) =>
    Effect.gen(function* () {
      const result = yield* dewcode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ dewcode }) =>
    Effect.gen(function* () {
      const result = yield* dewcode.spawn(["--mini", "--port", "4096"])
