...
// terminal sizes. The default dewcode tui command is excluded —
// `dewcode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `dewcode --help` prints today;
...
// top-level help on `--help` and exits 1; not a real dewcode command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("dewcode CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ dewcode }) =>
      Effect.gen(function* () {
        const topLevel = yield* dewcode.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* dewcode.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`dewcode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`dewcode ${argv.join(" ")} --help`)
        }
