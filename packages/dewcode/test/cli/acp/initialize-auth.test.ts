...
describe("dewcode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ dewcode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ dewcode }))
        ...
        expect(initialized.agentInfo?.name).toBe("DEWCode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ dewcode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ dewcode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("dewcode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "dewcode-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.DEWCODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ dewcode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("dewcode-login")
