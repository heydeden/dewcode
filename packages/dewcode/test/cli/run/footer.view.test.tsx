...
import type { QuestionRequest } from "@dewcode-ai/sdk/v2"
import { OpencodeKeymapProvider, registerOpencodeKeymap } from "@dewcode-ai/tui/keymap"
import {
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "dewcode", modelID: "gpt-5" },
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "dewcode", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("dewcode")
    expect(frame).toContain("GPT-5")
