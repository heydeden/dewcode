// Subprocess integration tests for `dewcode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `dewcode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `DEWCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("dewcode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, dewcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dewcode.run("say hi")
        dewcode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, dewcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dewcode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, dewcode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* dewcode.run("think", { extraArgs: ["--thinking"] })
        dewcode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* dewcode.run("think again")
        dewcode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, dewcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dewcode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, dewcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dewcode.run("say hi", { format: "json" })
        dewcode.expectExit(result, 0)

        const events = dewcode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ dewcode }) =>
      Effect.gen(function* () {
        const result = yield* dewcode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, dewcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dewcode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, dewcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dewcode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, dewcode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* dewcode.run("request permission", { permission: { bash: "ask" } })
        dewcode.expectExit(denied, 0)
        ...
        const allowed = yield* dewcode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        dewcode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* dewcode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        dewcode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, dewcode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* dewcode.serve()

        const result = yield* dewcode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, dewcode }) =>
      Effect.gen(function* () {
        const result = yield* dewcode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, dewcode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* dewcode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
