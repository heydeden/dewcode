...
describe("dewcode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // DEWCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ dewcode }) =>
      Effect.gen(function* () {
        const r = yield* dewcode.spawn(["mcp", "list"])
        dewcode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ dewcode }) =>
      Effect.gen(function* () {
        const r = yield* dewcode.spawn(["providers", "list"])
        dewcode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ dewcode }) =>
      Effect.gen(function* () {
        const r = yield* dewcode.spawn(["models"])
        dewcode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ dewcode }) =>
      Effect.gen(function* () {
        const r = yield* dewcode.spawn(["agent", "list"])
        dewcode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh DEWCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ dewcode }) =>
      Effect.gen(function* () {
        const r = yield* dewcode.spawn(["session", "list"])
        dewcode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ dewcode }) =>
      Effect.gen(function* () {
        const r = yield* dewcode.spawn(["stats"])
        dewcode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under DEWCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ dewcode }) =>
      Effect.gen(function* () {
        const r = yield* dewcode.spawn(["db", "path"])
        dewcode.expectExit(r, 0, "db path")
