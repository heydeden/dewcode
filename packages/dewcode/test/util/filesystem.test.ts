...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "dewcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "dewcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "dewcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "dewcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["dewcode.json", "dewcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "dewcode.json"),
        path.join(project, "dewcode.jsonc"),
      ])
    })
  })
