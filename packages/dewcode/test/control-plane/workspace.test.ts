...
import { EventSequenceTable } from "@dewcode-ai/core/event/sql"
...
import { WorkspaceV2 } from "@dewcode-ai/core/workspace"
import { WorkspaceTable } from "@dewcode-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  DEWCODE_AUTH_CONTENT: process.env.DEWCODE_AUTH_CONTENT,
  DEWCODE_EXPERIMENTAL_WORKSPACES: process.env.DEWCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.DEWCODE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@dewcode.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.DEWCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=dewcode-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.DEWCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.DEWCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.DEWCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=dewcode-test")
