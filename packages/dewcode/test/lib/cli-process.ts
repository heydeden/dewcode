// Subprocess test harness for the dewcode CLI. Spawns the real binary against
...
//
// Configuration flows through dewcode's built-in test affordances:
//   - DEWCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - DEWCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - DEWCODE_DISABLE_PROJECT_CONFIG : skip walking up for dewcode.json
//   - DEWCODE_PURE                : skip external plugin discovery + install
//   - DEWCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `dewcode.run` is fully wired. The shape supports adding more
// builders (`dewcode.serve(opts)`, `dewcode.acp(opts)`, `dewcode.auth(...)`)
...
import { AppProcess } from "@dewcode-ai/core/process"
...
const dewcodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(dewcodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    DEWCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    DEWCODE_CONFIG_CONTENT: configJson,
    DEWCODE_DISABLE_PROJECT_CONFIG: "1",
    DEWCODE_PURE: "1",
    DEWCODE_DISABLE_AUTOUPDATE: "1",
    DEWCODE_DISABLE_AUTOCOMPACT: "1",
    DEWCODE_DISABLE_MODELS_FETCH: "1",
    DEWCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `dewcode run`. New flags should
...
// `dewcode serve` is a long-lived process — it never exits on its own.
...
// `dewcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `dewcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `dewcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("dewcode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          DEWCODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("dewcode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("dewcode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "dewcode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `dewcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("dewcode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const dewcode: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, dewcode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `dewcode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "dewcode") {
...
// every call site. Use this for any test that needs the dewcode CLI fixture.
//
...
// resources (e.g. `dewcode.serve`) without an extra `Effect.scoped` wrapper —
