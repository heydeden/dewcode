...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const dewcodeFile = path.join(tmp, ".git", "dewcode")
      expect(yield* Effect.promise(() => Bun.file(dewcodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "dewcode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "dewcode")
      const wrongCache = path.join(parentDir, ".git", "dewcode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "dewcode")
      const cacheB = path.join(bareB, "dewcode")
      const wrongCache = path.join(parentDir, ".git", "dewcode")
