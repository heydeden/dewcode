...
it.instance("plan agent denies edits except .dewcode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".dewcode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".dewcode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.DEWCODE_TEST_HOME
      process.env.DEWCODE_TEST_HOME = test.directory
