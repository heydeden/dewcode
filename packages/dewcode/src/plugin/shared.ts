...
export const DEPRECATED_PLUGIN_PACKAGES = ["dewcode-openai-codex-auth", "dewcode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, dewcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(dewcodeVersion) || semver.major(dewcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.dewcode
  if (typeof range !== "string") return
  if (!semver.satisfies(dewcodeVersion, range)) {
    throw new Error(`Plugin requires dewcode ${range} but running ${dewcodeVersion}`)
  }
}
