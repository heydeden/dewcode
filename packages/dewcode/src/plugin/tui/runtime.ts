...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@dewcode-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@dewcode-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@dewcode-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@dewcode-ai/tui/context/theme"
import { Global } from "@dewcode-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@dewcode-ai/core/util/flock"
import { Flag } from "@dewcode-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@dewcode-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@dewcode-ai/core/v1/config/plugin"
import { createCommandShim } from "@dewcode-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@dewcode-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".dewcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".dewcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".dewcode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.DEWCODE_PURE ? [] : pluginOrigins
    if (Flag.DEWCODE_PURE && pluginOrigins.length) {
    }
