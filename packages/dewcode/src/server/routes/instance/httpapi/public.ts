...
import { DEWCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for dewcode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(DEWCodeHttpApi)
...
export const PublicApi = DEWCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "dewcode",
    version: "1.0.0",
    description: "dewcode api",
