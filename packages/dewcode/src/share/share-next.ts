...
import type * as SDK from "@dewcode-ai/sdk/v2"
...
import { SessionShareTable } from "@dewcode-ai/core/share/sql"
...
const disabled = process.env["DEWCODE_DISABLE_SHARE"] === "true" || process.env["DEWCODE_DISABLE_SHARE"] === "1"
...
export class Service extends Context.Service<Service, Interface>()("@dewcode/ShareNext") {}
  ...
  Effect.gen(function* () {
    ...
    const request = Effect.fn("ShareNext.request")(function* () {
      ...
      if (Option.isNone(active) || !active.value.active_org_id) {
        const baseUrl = (yield* cfg.get()).enterprise?.url ?? "https://dewcode.dev"
