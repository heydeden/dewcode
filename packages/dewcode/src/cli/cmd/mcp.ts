...
import { ConfigMCPV1 } from "@dewcode-ai/core/v1/config/mcp"
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: dewcode mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in dewcode.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .dewcode/ subdirectory too)
  const candidates = [path.join(baseDir, "dewcode.json"), path.join(baseDir, "dewcode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".dewcode", "dewcode.json"), path.join(baseDir, ".dewcode", "dewcode.jsonc"))
  }
  ...
  // Default to dewcode.json if none exist
  return candidates[0]
}
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., dewcode x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
...
export const McpDebugCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.debug")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const client = new Client({ name: "dewcode-debug", version: InstallationVersion }, CLIENT_OPTIONS)
