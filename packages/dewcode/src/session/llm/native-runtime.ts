...
import type { LLMClientShape } from "@dewcode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("dewcode"))
    return { type: "unsupported", reason: "provider is not openai, dewcode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @dewcode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains dewcode-owned. The native runtime only adapts
      // the @dewcode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
