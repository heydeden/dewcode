...
import { isRecord } from "@dewcode-ai/tui/util/record"
...
import { TuiKeybind } from "@dewcode-ai/tui/config/keybind"
...
import { TuiConfig } from "@dewcode-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@dewcode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old dewcode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.dewcode`
  // folders between cwd and home, and DEWCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.DEWCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit DEWCODE_TUI_CONFIG override, if set.
  if (Flag.DEWCODE_TUI_CONFIG) {
    const configFile = Flag.DEWCODE_TUI_CONFIG
  ...
  // 4. `.dewcode` directories (and DEWCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".dewcode") || dir === Flag.DEWCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".dewcode") && dir !== Flag.DEWCODE_CONFIG_DIR) continue
