...
const TUI_SCHEMA_URL = "https://dewcode.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from dewcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const dewcode = await dewcodeFiles(input)
  for (const file of dewcode) {
...
async function dewcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "dewcode"),
    ...(await Filesystem.findUp(["dewcode.json", "dewcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "dewcode"))
  }
  if (Flag.DEWCODE_CONFIG) files.push(Flag.DEWCODE_CONFIG)
