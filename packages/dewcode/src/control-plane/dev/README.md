This is a plugin to simulate a remote environment locally. Add this to `.dewcode/dewcode.jsonc`:

```json
  "plugin": ["../packages/dewcode/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate DEWCode server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/dewcode/script/run-workspace-server
```

With the plugin install, you can now run DEWCode and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
