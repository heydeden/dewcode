...
import type { AssistantMessage, Message, OpencodeClient, SessionMessageResponse } from "@dewcode-ai/sdk/v2"
...
export const AuthMethodID = "dewcode-login"
...
export class Service extends Context.Service<Service, Interface>()("@dewcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `dewcode auth login` in the terminal",
      name: "Login with dewcode",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, dewcode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const dewcodeProvider = providers[ProviderV2.ID.make("dewcode")]
  const dewcodeModel = dewcodeProvider ? Provider.sort(Object.values(dewcodeProvider.models))[0] : undefined
  if (dewcodeProvider && dewcodeModel) return { providerID: dewcodeProvider.id, modelID: dewcodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "DEWCode service failure", service })
}
