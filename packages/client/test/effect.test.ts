...
import { AbsolutePath, Agent, Location, Model, DEWCode, Prompt, Session, SessionMessage } from "../src/effect"
...
test("sessions.get returns the decoded Effect projection", async () => {
  ...
  const result = await Effect.gen(function* () {
    const client = yield* DEWCode.make({ baseUrl: "http://localhost:3000" })
