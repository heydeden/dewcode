...
const directory = "C:/DEWCode/FileBrowserSidebar"
...
async function setup(page: Page) {
  await mockDEWCodeServer(page, {
    ...
    provider: {
      ...
      connected: ["dewcode"],
      default: { providerID: "dewcode", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "dewcode.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "dewcode.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "dewcode.window.browser.dat:tabs",
