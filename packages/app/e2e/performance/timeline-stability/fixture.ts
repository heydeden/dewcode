...
import { SessionStatusEvent } from "@dewcode-ai/schema/session-status-event"
import { SessionV1 } from "@dewcode-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/DEWCode/TimelineStability"
...
export const model = { providerID: "dewcode", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("dewcode.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["dewcode"],
    default: { providerID: "dewcode", modelID: "claude-opus-4-6" },
  }
}
