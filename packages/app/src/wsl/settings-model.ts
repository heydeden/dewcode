...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-dewcode" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslOpencodeAction(check?: WslOpencodeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install DEWCode"
  if (check.matchesDesktop === false) return "Update DEWCode"
}
...
export function addServerViewModel(input: {
  ...
  const dewcodeCheck = selectedDistro ? (state?.dewcodeChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.dewcodeChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.dewcodeMissing" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  dewcodeCheck: WslOpencodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingOpencode =
    input.state?.job?.kind === "install-dewcode" && input.state.job.distro === input.selectedDistro
  ...
  if (!addServerOpencodeReady(input.dewcodeCheck)) {
    const update = !!input.dewcodeCheck?.resolvedPath && input.dewcodeCheck.matchesDesktop === false
    return {
      variant: "neutral",
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.dewcodeChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.dewcodeChecks[item.name]) return [`dewcode:${item.name}`]
    return []
  })
