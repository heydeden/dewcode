...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when DEWCode needs attention", () => {
    ...
    ).toBe("Install DEWCode")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/dewcode",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when DEWCode is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        dewcodeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.dewcode/bin/dewcode",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.primaryButton.action).toBe("install-dewcode")
  })
