    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.dewcodeChecks[item.config.distro]
          const dewcodeAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-dewcode" && wsl.data.job.distro === item.config.distro
          return (
