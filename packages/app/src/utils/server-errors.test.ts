...
import type { SessionNotFoundError } from "@dewcode-ai/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em dewcode.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
