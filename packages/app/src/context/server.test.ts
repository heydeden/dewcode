  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://dewcode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://dewcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://dewcode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://dewcode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://dewcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://dewcode.example.com"),
      ),
    ).toEqual({
