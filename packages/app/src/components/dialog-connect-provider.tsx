...
import { IconButton } from "@dewcode-ai/ui/icon-button"
import { List, type ListRef } from "@dewcode-ai/ui/list"
import { ProviderIcon } from "@dewcode-ai/ui/provider-icon"
import { Spinner } from "@dewcode-ai/ui/spinner"
import { Tag } from "@dewcode-ai/ui/tag"
import { TextField } from "@dewcode-ai/ui/text-field"
import { ButtonV2 } from "@dewcode-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@dewcode-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@dewcode-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "dewcode-go") return language.t("dialog.provider.dewcodeGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "dewcode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.dewcode.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["dewcode", "dewcode-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "dewcode" || provider.id === "dewcode-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "dewcode"
                                ? "dialog.provider.dewcode.tagline"
                                : "dialog.provider.dewcodeGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "dewcode"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.dewcodeZen.line1")}</div>
              <div>{language.t("provider.connect.dewcodeZen.line2")}</div>
              <div>
                {language.t("provider.connect.dewcodeZen.visit.prefix")}
                <Link
                  href="https://dewcode.dev/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.dewcodeZen.visit.link")}
                </Link>
                {language.t("provider.connect.dewcodeZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "dewcode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.dewcodeZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.dewcodeZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.dewcodeZen.visit.prefix")}
                <Link href="https://dewcode.dev/zen" tabIndex={-1}>
                  {language.t("provider.connect.dewcodeZen.visit.link")}
                </Link>
                {language.t("provider.connect.dewcodeZen.visit.suffix")}
              </div>
