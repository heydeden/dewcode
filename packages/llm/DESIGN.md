...
> current private `@dewcode-ai/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@dewcode-ai/ai`
...
- Secondary audience: DEWCode and other durable agent runtimes
...
import { LLM } from "@dewcode-ai/ai"
import { OpenAI } from "@dewcode-ai/ai/providers/openai"
...
```ts
import { Message } from "@dewcode-ai/ai"
...
import { LLM, Tool } from "@dewcode-ai/ai"
...
DEWCode and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@dewcode-ai/ai/promise"
import { OpenAI } from "@dewcode-ai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@dewcode-ai/ai/schema"
...
```ts
import { Provider, Protocol } from "@dewcode-ai/ai/provider"
...
@dewcode-ai/ai/promise
  Promise/AsyncIterable LLM facade

@dewcode-ai/ai/schema
  serializable domain schemas

@dewcode-ai/ai/provider
  experimental Provider and Protocol authoring APIs

@dewcode-ai/ai/providers/openai
@dewcode-ai/ai/providers/anthropic
@dewcode-ai/ai/providers/google
...
```
...
DEWCode should migrate to `generateTurn` / `streamTurn`, preserving its durable
