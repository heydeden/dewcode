...
import { createOpencode, type ToolPart } from "@dewcode-ai/sdk"
...
console.log("🚀 Starting dewcode server...")
const dewcode = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await dewcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new dewcode session...")
    const { client, server } = dewcode
    ...
    console.log("✅ Created dewcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to dewcode:", message.text)
