...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve dewcode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "dewcode")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "dewcode")).toBe("openai")
    expect(statProvider("big-pickle", "", "dewcode")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "dewcode"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep dewcode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "dewcode"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "dewcode"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep dewcode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "dewcode"), country: "US" })).toMatchObject([
