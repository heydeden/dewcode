...
This is the living design and status document for `@dewcode-ai/codemode` and its existing V2 DEWCode adapter.
...
`packages/dewcode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
