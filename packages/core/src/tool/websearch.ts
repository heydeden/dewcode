...
import { ToolFailure } from "@dewcode-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@dewcode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.DEWCODE_WEBSEARCH_PROVIDER === "exa" || process.env.DEWCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.DEWCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("DEWCODE_EXPERIMENTAL") || truthy("DEWCODE_ENABLE_EXA") || truthy("DEWCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("DEWCODE_ENABLE_PARALLEL") || truthy("DEWCODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `dewcode/${InstallationVersion}`,
