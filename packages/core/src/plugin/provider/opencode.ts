...
import type { IntegrationOAuthMethodRegistration } from "@dewcode-ai/plugin/v2/effect/integration"
import { define } from "@dewcode-ai/plugin/v2/effect/plugin"
...
const defaultServer = "https://console.dewcode.ai"
const clientID = "dewcode-cli"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("dewcode")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load DEWCode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("dewcode", (integration) => {
        integration.name = "DEWCode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "dewcode", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("dewcode")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("dewcode")
      ...
      const item = catalog.provider.get(ProviderV2.ID.dewcode)
      if (!item) return
      const hasKey = Boolean(process.env.DEWCODE_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("dewcode")),
