...
import { define } from "@dewcode-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@dewcode-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "dewcode.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "dewcode.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
