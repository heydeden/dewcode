...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@dewcode/acme")).toBe("@dewcode/acme")
    expect(Npm.sanitize("@dewcode/acme@1.0.0")).toBe("@dewcode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/dewcode/acme.git"
    const expected = win ? "acme@git+https_//github.com/dewcode/acme.git" : spec
