...
import { GoogleVertexPlugin } from "@dewcode-ai/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.dewcode, (provider) => {
          provider.api = {
            ...
            url: "https://dewcode.dev/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.dewcode))
