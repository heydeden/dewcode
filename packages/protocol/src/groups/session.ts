...
import { PromptInput } from "@dewcode-ai/schema/prompt-input"
import { Session } from "@dewcode-ai/schema/session"
import { Project } from "@dewcode-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@dewcode-ai/schema/schema"
...
import { Revert } from "@dewcode-ai/schema/revert"
import { SessionEvent } from "@dewcode-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this DEWCode process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this DEWCode process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
