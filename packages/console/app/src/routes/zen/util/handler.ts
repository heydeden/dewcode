...
import { and, Database, eq, isNull, lt, or, sql } from "@dewcode-ai/console-core/drizzle/index.js"
import { KeyTable } from "@dewcode-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@dewcode-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@dewcode-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@dewcode-ai/console-core/util/date.js"
import { Identifier } from "@dewcode-ai/console-core/identifier.js"
import { Billing } from "@dewcode-ai/console-core/billing.js"
import { Actor } from "@dewcode-ai/console-core/actor.js"
import { WorkspaceTable } from "@dewcode-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@dewcode-ai/console-core/model.js"
import { Subscription } from "@dewcode-ai/console-core/subscription.js"
import { BlackData } from "@dewcode-ai/console-core/black.js"
import { UserTable } from "@dewcode-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@dewcode-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@dewcode-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@dewcode-ai/console-core/lite.js"
import { Resource } from "@dewcode-ai/console-resource"
...
import { Workspace } from "@dewcode-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-dewcode-session") ?? ""
    const requestId = input.request.headers.get("x-dewcode-request") ?? ""
    const ocClient = input.request.headers.get("x-dewcode-client") ?? ""
    const projectId = input.request.headers.get("x-dewcode-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://dewcode.dev/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-dewcode-request")
            headers.delete("x-dewcode-session")
            headers.delete("x-dewcode-project")
            headers.delete("x-dewcode-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-dewcode-endpoint-id")
        const resEndpointModelId = res.headers.get("x-dewcode-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://dewcode.dev/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://dewcode.dev/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://dewcode.dev/workspace/${authInfo.workspaceID}/members`
