...
import type { UpdaterState } from "@dewcode-ai/app/updater"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.DEWCODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "dewcode:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__DEWCODE__ ??= {}
  const pending = window.__DEWCODE__.deepLinks ?? []
  window.__DEWCODE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `dewcode.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://dewcode.dev/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("dewcode.global.dat").getItem("language")
