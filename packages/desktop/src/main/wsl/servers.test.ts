...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "DEWCode update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating DEWCode", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.dewcode/bin/dewcode",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, dewcodeChecks: {} })
})
...
test("ignores stale background DEWCode checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().dewcodeChecks).toEqual({})
})

test("ignores stale startup DEWCode checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking DEWCode", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const dewcode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveOpencode: async (distro) => {
      dewcode.push(distro)
      return "/home/me/.dewcode/bin/dewcode"
    },
  })
  ...
  expect(dewcode).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(dewcode).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().dewcodeChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check DEWCode in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const dewcode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(dewcode).toEqual(["Debian"])
  expect(Object.keys(controller.getState().dewcodeChecks)).toEqual(["Debian"])
})
