...
export function createWslServersController(
  ...
  const checkOpencode = async (distro: string, opts?: { signal?: AbortSignal }) => {
    ...
    const version = resolved
      ...
      : null
    return dewcodeCheck(distro, resolved, version, appVersion)
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const dewcodeChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.dewcodeChecks[distro])
        .map(async (distro) => [distro, await checkOpencode(distro, opts)] as const),
    )
    if (dewcodeChecks.length) {
      setState({ dewcodeChecks: { ...state.dewcodeChecks, ...Object.fromEntries(dewcodeChecks) } })
    }
  }
  ...
  const refreshOpencodeCheckBackground = (id: string, distro: string) => {
    void checkOpencode(distro)
      ...
      .catch((error) => {
        ...
        logger?.error("wsl dewcode check failed", { id, distro, message })
      })
  }
    ...
    await Promise.all(
      state.servers.map((item) =>
        checkOpencode(item.config.distro)
          ...
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl dewcode check failed", {
              id: item.config.id,
  ...
  return {
    ...
    async installOpencode(name: string) {
      await runJob({ kind: "install-dewcode", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslOpencode(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "DEWCode installation failed")
        }
        await refreshOpencodeCheck(name, { signal: abort.signal })
        expectOpencodeVersion(state.dewcodeChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.dewcodeChecks, distro) : {}),
      })
    },
...
function dewcodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "dewcode is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "dewcode is installed but could not run",
    }
  }
  return {
    distro,
