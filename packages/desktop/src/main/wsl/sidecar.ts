...
export async function spawnWslSidecar(
  ...
  const dewcode = await resolveWslOpencode(distro)
  if (!dewcode) throw new Error(`DEWCode is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "dewcode"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export DEWCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export DEWCODE_CLIENT=desktop",
    `export DEWCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export DEWCODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(dewcode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
