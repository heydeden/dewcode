...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier DEWCode launch", () => {
    expect(hasExistingAppState([file("dewcode.settings")])).toBe(true)
    expect(hasExistingAppState([file("dewcode.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("dewcode")])).toBe(true)
  })
})
