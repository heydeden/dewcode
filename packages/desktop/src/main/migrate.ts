...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.dewcode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `dewcode.settings.dat` is special: it maps to the `dewcode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"dewcode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // dewcode.settings.dat → the electron settings store ("dewcode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "dewcode.global.dat").
  const storeName = filename === "dewcode.settings.dat" ? "dewcode.settings" : filename
