...
import { resolveThemeVariant } from "@dewcode-ai/ui/theme/resolve"
import type { DesktopTheme } from "@dewcode-ai/ui/theme/types"
...
function windowDataFile(id: string) {
  return `dewcode.window.${id.replace(/[^a-zA-Z0-9._-]/g, "-")}.dat`
}
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      "DEWCode window terminated unexpectedly",
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("DEWCode is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
