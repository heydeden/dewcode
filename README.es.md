...
npm i -g dewcode-ai@latest        # o bun/pnpm/yarn
scoop install dewcode             # Windows
choco install dewcode             # Windows
brew install anomalyco/tap/dewcode # macOS y Linux (recomendado, siempre al día)
brew install dewcode              # macOS y Linux (fórmula oficial de brew, se actualiza menos)
sudo pacman -S dewcode            # Arch Linux (Stable)
paru -S dewcode-bin               # Arch Linux (Latest from AUR)
mise use -g dewcode               # cualquier sistema
nix run nixpkgs#dewcode           # o github:dedenwirjadinata/dewcode para la rama dev más reciente
```
...
DEWCode también está disponible como aplicación de escritorio. Descárgala directamente desde la [página de releases](https://github.com/dedenwirjadinata/dewcode/releases) o desde [dewcode.dev/download](https://dewcode.dev/download).
...
1. `$DEWCODE_INSTALL_DIR` - Directorio de instalación personalizado
2. `$XDG_BIN_DIR` - Ruta compatible con la especificación XDG Base Directory
3. `$HOME/bin` - Directorio binario estándar del usuario (si existe o se puede crear)
4. `$HOME/.dewcode/bin` - Alternativa por defecto
...
DEWCode incluye dos agentes integrados que puedes alternar con la tecla `Tab`.
...
Más información sobre [agentes](https://dewcode.dev/docs/agents).

### Documentación

Para más información sobre cómo configurar DEWCode, [**ve a nuestra documentación**](https://dewcode.dev/docs).

### Contribuir

Si te interesa contribuir a DEWCode, lee nuestras [docs de contribución](./CONTRIBUTING.md) antes de enviar un pull request.

### Proyectos basados en DEWCode

Si estás trabajando en un proyecto basado en DEWCode y usas "dewcode" como parte del nombre, por ejemplo, "dewcode-dashboard" u "dewcode-mobile", agrega una nota en tu README para aclarar que no está hecho por el equipo de DEWCode y que no está afiliado con nosotros de ninguna manera.

---

**Únete a nuestra comunidad** [Discord](https://discord.gg/dewcode) | [X.com](https://x.com/dewcode)
