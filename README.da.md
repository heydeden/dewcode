...
brew install anomalyco/tap/dewcode # macOS og Linux (anbefalet, altid up to date)
brew install dewcode              # macOS og Linux (officiel brew formula, opdateres sjældnere)
...
DEWCode findes også som desktop-app. Download direkte fra [releases-siden](https://github.com/dedenwirjadinata/dewcode/releases) eller [dewcode.dev/download](https://dewcode.dev/download).
...
1. `$DEWCODE_INSTALL_DIR` - Tilpasset installationsmappe
...
DEWCode har to indbyggede agents, som du kan skifte mellem med `Tab`-tasten.
...
Læs mere om [agents](https://dewcode.dev/docs/agents).

### Dokumentation

For mere info om konfiguration af DEWCode, [**se vores docs**](https://dewcode.dev/docs).

### Bidrag

Hvis du vil bidrage til DEWCode, så læs vores [contributing docs](./CONTRIBUTING.md) før du sender en pull request.

### Bygget på DEWCode

Hvis du arbejder på et projekt der er relateret til DEWCode og bruger "dewcode" som en del af navnet; f.eks. "dewcode-dashboard" eller "dewcode-mobile", så tilføj en note i din README, der tydeliggør at projektet ikke er bygget af DEWCode-teamet og ikke er tilknyttet os på nogen måde.

---

**Bliv en del af vores community** [Discord](https://discord.gg/dewcode) | [X.com](https://x.com/dewcode)
