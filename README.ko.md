...
# 패키지 매니저
npm i -g dewcode-ai@latest        # bun/pnpm/yarn 도 가능
scoop install dewcode             # Windows
choco install dewcode             # Windows
brew install anomalyco/tap/dewcode # macOS 및 Linux (권장, 항상 최신)
brew install dewcode              # macOS 및 Linux (공식 brew formula, 업데이트 빈도 낮음)
sudo pacman -S dewcode            # Arch Linux (Stable)
paru -S dewcode-bin               # Arch Linux (Latest from AUR)
mise use -g dewcode               # 어떤 OS든
nix run nixpkgs#dewcode           # 또는 github:dedenwirjadinata/dewcode 로 최신 dev 브랜치
```
...
DEWCode 는 데스크톱 앱으로도 제공됩니다. [releases page](https://github.com/dedenwirjadinata/dewcode/releases) 에서 직접 다운로드하거나 [dewcode.dev/download](https://dewcode.dev/download) 를 이용하세요.
...
DEWCode 에는 내장 에이전트 2개가 있으며 `Tab` 키로 전환할 수 있습니다.
...
[agents](https://dewcode.dev/docs/agents) 에 대해 더 알아보세요.

### 문서

DEWCode 설정에 대한 자세한 내용은 [**문서**](https://dewcode.dev/docs) 를 참고하세요.

### 기여하기

DEWCode 에 기여하고 싶다면, Pull Request 를 제출하기 전에 [contributing docs](./CONTRIBUTING.md) 를 읽어주세요.

### DEWCode 기반으로 만들기

DEWCode 와 관련된 프로젝트를 진행하면서 이름에 "dewcode"(예: "dewcode-dashboard" 또는 "dewcode-mobile") 를 포함한다면, README 에 해당 프로젝트가 DEWCode 팀이 만든 것이 아니며 어떤 방식으로도 우리와 제휴되어 있지 않다는 점을 명시해 주세요.

---

**커뮤니티에 참여하기** [Discord](https://discord.gg/dewcode) | [X.com](https://x.com/dewcode)
