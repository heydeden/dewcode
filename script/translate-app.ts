...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "dewcode/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".dewcode/glossary/zh-cn.md"
  if (locale === "zht") return ".dewcode/glossary/zh-tw.md"
  return `.dewcode/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("DEWCode did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("DEWCode returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("DEWCode session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel DEWCode runs for 'all' (default: 4)
      --model <provider/id>  DEWCode model (default: dewcode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running DEWCode
...
`)
  ...
  if (failed.length) console.error(`\nDEWCode failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.DEWCODE_DISABLE_PROJECT_CONFIG = "1"
  env.DEWCODE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["dewcode", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.DEWCODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["dewcode", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.DEWCODE_CONFIG
  delete env.DEWCODE_CONFIG_DIR
  delete env.DEWCODE_CONFIG_CONTENT
  delete env.DEWCODE_PERMISSION
  delete env.DEWCODE_AUTO_SHARE
  return env
}
