...
import { createOpencode } from "@dewcode-ai/sdk"
...
`)
  ...
  const dewcode = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await dewcode.client.session.create()
    const result = await dewcode.client.session
      .prompt({
