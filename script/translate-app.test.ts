...
describe("translate app", () => {
  ...
  test("maps product locale codes to their glossaries", () => {
    expect(glossaryFile("fr")).toBe(".dewcode/glossary/fr.md")
    expect(glossaryFile("zh")).toBe(".dewcode/glossary/zh-cn.md")
    expect(glossaryFile("zht")).toBe(".dewcode/glossary/zh-tw.md")
  })
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "dewcode/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
...
{"variants":{}}
`
    expect(modelVariants(output, "dewcode/gpt-5.5")).toEqual({
  ...
  test("disables side effects and scopes edits for the translation agent", () => {
    const config = translationConfig("translate-app-fr", "dewcode/gpt-5.5", ["packages/app/src/i18n/fr.ts"])
